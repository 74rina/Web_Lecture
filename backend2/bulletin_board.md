...
入門2では、掲示板のバックエンドを実装しながら、動的サイトの主役である「API設計」について学びます。
...
このテーブル設計が、`schema.sql`内に次のように書かれている。
...
4. 【演習】`users`テーブルに`骨つきチキン/test2`が追加されたか、`4-1`または`4-2`の方法で確認してみよう！
...
- ボディに次を含める。
  ...
  →なぜか400エラー。curlでは行けるので、read_json_body()がよくない？

...
1.  リポジトリのクローン
    ...
    ```
    $ git clone https://github.com/74rina/Bulletin_Board.git
    ```
...
DB設計において、テーブル構造を可視化するために用いる。エンティティ（テーブル）・エンティティ同士のリレーション・アトリビュート（各カラム）を図示する。
:::
...
- **`-d`**（data）
  ...
  ex.) `$ curl -d "name=test" https://example.com/register`

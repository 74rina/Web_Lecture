...
1. **アプリケーション**：作ったWebアプリ、書いたソースコードなど
...
1. Docker Desktop のインストール
   ...
   自分のOSおよびCPUアーキに合わせて、インストーラーをダウンロードする。
...
5. サインインした状態で、Docker Desktop に飛ぶ
...
Webサーバとは、クライアントからHTTPリクエストを受け取り、HTMLなどの静的ファイルを返すコンピュータのこと（バックエンド入門１を参照）。
...
※ Dockerイメージ ＝ Dockerコンテナを作る（環境を丸ごと再現する）ための設計図。
...
を実行し、ブラウザで8080番にアクセスする（先ほどの`my-nginx`コンテナを削除するか、コンテナ名が衝突しないようにする。**ポート競合**にも注意）。
...
バックエンド入門２の「掲示板」を、Docker上で動かすことを考える。

前：`$ php -S localhost:8000` でPHPのビルトインWebサーバを立てていた

後：`$ docker compose up --build` で実行環境を構築する（環境による差異×）
...
※ ビルド ＝ Dockerfileを元にDockerイメージを作ること。

上から順に環境を組み立て、最後に何を実行するか書く。
...
- `RUN`
  イメージのビルド時に実行するコマンド。パッケージのインストール、ライブラリの追加など。
- `CMD`
  コンテナの起動時に実行するコマンド。
...
Web（`nginx`）、AP（`php-fpm`）、DB（`mysql`）の3コンテナを動かすので、
...
- ローカルの `db_data` ディレクトリを、コンテナ内では `/var/lib/mysql` と表すとする（一般的に **`<ホストのリソース>:<ゲストのリソース>`** と書く）。
...
- 使用するイメージは、既存イメージではなく、Dockerfile で自作したものなので、`image`ではなく`build`で指定する。
...
- `$ docker compose build`, `$ docker compose up` の2回に分けても良い。
  Dockerfile を更新していない・既存のイメージがあるなら `$ docker compose up` だけで良い。

- 起動だけならGUIが楽（Docker Desktopの各コンテナの再生ボタンを押す）。
...
- **起動中のコンテナの中に入る**（コンテナ内のシェルでコマンドを打てる）

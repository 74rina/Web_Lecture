...
[参考リポジトリ](https://github.com/74rina/bulletin_board_2)
...
開発（Development）と運用・保守（Operations）を分断せず、高品質なソフトウェアを迅速＋継続的にリリースする手法。
...
→ これらのライブラリの詰め合わせ。Python系は、Django, Flaskなど。
:::
...
1. **アプリケーションのビルド**
   ...
   ::: warning
   トラブルシューティングとして、コンテナ・ボリュームの初期化

   `$ docker compose down -v`

   を考えても良いが、ボリューム（DBの中身）が全て消えることに注意！
...
を行うだけで良い。既存のマイグレーションファイル通りに、SQLクエリが実行される。
...
APIで値をやり取りする際の**型定義**のこと。「このAPIはこの型のデータを扱う」という定義。
...
- Qiita [JavaScriptにおける非同期処理とは](https://qiita.com/Hirohana/items/6a7fd4c7f4c9ccb1f6da)

...
- そもそもWeb開発を独学しようとしても、何から手をつければよいのかわからない

そこで、エンジニアを目指し始めて同じ悩みを抱える方が、Web開発の基礎を体系的に学び、その後スムーズに実践経験へとつなげていけるよう、本記事を執筆しています。
...
`ファイル` > `新しいファイル` から、Pythonファイル（`test.py`）を作っておく。

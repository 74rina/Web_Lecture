...
`go run main.go`
で、スクリプトみたいに実行できる
内部では、コンパイル→バイナリ生成→実行をやってる。

go コマンド＝開発に必要ないろいろなツールを提供してる
...
# 5. Go言語で WebSocket を実装する

## goroutine とは

---

`github.com/zishang520/socket.io/socket`ライブラリを使用。

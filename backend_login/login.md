...
静的サイトだけでなく、ユーザの操作に応じて内容が変わる、**動的サイト**を作ることを目指します。

今回は、ログイン処理の仕組みを理解することを目的とし、フレームワークは使わず、PHPで簡易的なログイン機能を実装します。
...
- プレゼンテーション層（UI）、ファンクション層/AP層（処理）、データベースアクセス層に分離
...
- Python系
  Gunicorn, uWSGI（WSGIサーバ）フレームワーク：Django, Flask
  Uvicorn（ASGIサーバ）フレームワーク：FastAPI

- Ruby系
  Puma, Unicorn（Rackサーバ）フレームワーク：Rails
...
1. CSR（Client Side Rendering）
   - リクエストを受けたサーバが、空のHTMLファイルと実行するJSを返す
   - ブラウザ側でJSが実行され、**DOM**が組み立てられ、レンダリングされる
...
WebサーバとWebクライアント（ブラウザ）間でWeb情報をやり取りするための通信プロトコル。1通信＝リクエストとレスポンスの1往復。
...
（セッション ＝ ログインしてからログアウトするまでの一連の通信のまとまり）
...
罠サイトからのリクエストでも、ログインユーザーのセッションIDが付いていれば、本人からのリクエストだと見なされてしまう（なりすまし）。成立条件は、①ユーザーがログイン中であること、②Cookie認証を使っていること。
...
- 3大要素
  - **カプセル化**：データと処理を一つにまとめ、内部の詳細を外部から隠蔽する
  - **インヘリタンス（継承）**：スーパークラス（親）で定義した属性やメソッドをサブクラス（子）が使える
  - **ポリモーフィズム（多態性）**：同じ操作で、オブジェクトごとに結果が異なる

- クラスとインスタンス
  類似オブジェクトに共通する性質を抜き出し、属性（変数）やメソッド（関数）を一般化して、**クラス**を定義する。それを使用して生成した実体（具体値を持ったオブジェクト）が**インスタンス**。
...
これを防ぐために、`?`プレースホルダを用いて、SQL構造とデータを分離する。
...
`login.php`でログイン成功 → `mypage.php`に遷移
（実際のログイン処理や認証は切り分ける）
...
`git checkout -b "feature/logout"` でブランチを切ることをおすすめします。
...
バックエンド入門（仮）ありがとうございました！

実践編としては、

- PHPを学んだので、Laravelフレームワークで動的サイトを作ってみる
- Python講習会の延長でFastAPIを学ぶ
- Docker を完全理解する
- DBの設計をきわめる
- ソフトウェア工学、アーキテクチャをちゃんと学ぶ（？）

などを考えています。今後も一緒に学んでくださったら嬉しいです！！！
